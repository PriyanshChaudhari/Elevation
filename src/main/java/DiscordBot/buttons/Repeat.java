...
public class Repeat extends Button {
  ...
  public void execute(ButtonInteractionEvent event) {
    ...
    if (music.isLoop()) {
      event.getInteraction().editButton(ButtonManager.repeat().withStyle(ButtonStyle.SUCCESS)).queue();
    } else {
      event.getInteraction().editButton(ButtonManager.repeat().withStyle(ButtonStyle.PRIMARY)).queue();
    }
  }
